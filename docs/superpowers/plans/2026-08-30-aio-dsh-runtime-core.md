...
**Goal:** 构建由 AIO Plugin API v3 托管、可销毁且可恢复的完整 DSH Coding 执行域，并以可复现离线包支持 Windows x64、Linux x64、macOS arm64，同时发布 Linux arm64 预览包。
...
**Tech Stack:** AIO Hub 0.7.0-alpha.4、Plugin API v3、Rust、Serde/JSON Schema、TypeScript ESM、Bun 1.3.11、Vitest 4、DSH `dsh-v0.1.2-alpha.1`（commit `cd5ef8148158c3a752a658978873241fdf8e2bbc`）、Node.js 22.19.0、pnpm 11.7.0、Python 3.10 runtime build baseline、GitHub Actions 原生 runners。
...
- Phase 1 supported 平台固定为 `win32-x64`、glibc 2.28+ 的 `linux-x64`、macOS 14+ 的 `darwin-arm64`；`linux-arm64` 只能标记 Preview；其他平台启动前明确拒绝。
- 每个平台独立 ZIP，首次启动完全离线，不要求系统安装 Node.js、Python、WSL2 或 DSH，也不允许首次运行下载 runtime。
- Runtime 基线固定为 DSH `dsh-v0.1.2-alpha.1` / `cd5ef8148158c3a752a658978873241fdf8e2bbc`；精确官方 wheel 优先，不可用时在原生 runner 从固定源码构建并标记 `project-built-from-official-source`。
...
- Create: `runtime-lock/dsh-v0.1.2-alpha.1.json` - 平台 artifact、provenance、hash、license、SBOM、contract/profile/AIO range。
- Create: `scripts/runtime/**`, `scripts/package-platform.ts`, `.github/workflows/ci.yml`, `.github/workflows/release-runtime.yml` - 官方 wheel 探测、固定源码回退、离线 closure、每平台 ZIP 与门禁。
...
在插件仓库初始化 Git，复制 `plugins/AGENTS.template.md` 内容到独立 `AGENTS.md`，写入 Apache-2.0 license、贡献说明和以下脚本：`check`、`test`、`build`、`generate:protocol`、`check:generated`、`runtime:resolve-current`、`package:platform`。`resolve-runtime.ts` 与 `package-platform.ts` 在省略 `--platform` 时必须从 `process.platform/process.arch` 严格解析当前支持键，不支持组合直接失败。`.node-version` 固定 `22.19.0`；`packageManager` 固定 `bun@1.3.11`；DSH 源码构建脚本另行固定 pnpm `11.7.0` 与 Python `3.10`。
...
it("declares a resident API v3 DSH execution domain on four packaged platforms", async () => {
  ...
  expect(Object.keys(manifest.sidecar.executable).sort()).toEqual(["darwin-arm64", "linux-arm64", "linux-x64", "win32-x64"]);
...
- [ ] **Step 3: 写 manifest、DTO 与 transport adapter**

manifest 中 `type` 为 `sidecar`、`host.apiVersion` 为 `3`，路径固定为 `bin/win32-x64/aio-dsh-supervisor.exe`、`bin/linux-x64/aio-dsh-supervisor`、`bin/darwin-arm64/aio-dsh-supervisor`、`bin/linux-arm64/aio-dsh-supervisor`；settings 只含 `prewarm: false` 与 `idleGraceSeconds: 600`；不声明 UI。`AioSidecarTransport` 只负责 API v3 `spawn/send/kill/event` 与 DTO 转换，不引入 Vue/Pinia 类型。
...
### Task 5: 可复现 DSH Runtime Lock 与产物流水线（OpenSpec 2.1-2.5）

**Files:**
- Create: `E:\workspace\projects\aiohub-plugin-dsh-workspace\runtime-lock\dsh-v0.1.2-alpha.1.json`
- Create: `E:\workspace\projects\aiohub-plugin-dsh-workspace\scripts\runtime\resolve-runtime.ts`
- Create: `E:\workspace\projects\aiohub-plugin-dsh-workspace\scripts\runtime\build-from-source.ts`
...
- Produces: `RuntimeLockV1`；`resolveRuntime(platform): Promise<VerifiedRuntime>`；source 仅为 `official-wheel` 或 `project-built-from-official-source`。

- [ ] **Step 1: 写恶意/不完整 lock 的失败测试**

```typescript
it.each(["moving-ref", "temporary-actions-artifact", "wrong-arch", "missing-helper", "checksum-mismatch", "unsupported-license", "contract-mismatch"])("rejects %s", async (fault) => {
  await expect(verifyRuntime(fixtureRuntime(fault))).rejects.toMatchObject({ code: `RUNTIME_${fault.toUpperCase().replaceAll("-", "_")}` });
});
```

- [ ] **Step 2: 运行测试并确认 resolver 缺失**

Run: `bunx vitest run scripts/runtime/runtime-lock.test.ts tests/e2e/runtime-artifact.test.ts`

Expected: FAIL，无法导入 `resolve-runtime`/`verify-runtime`。

- [ ] **Step 3: 实现 lock、官方 wheel 优先和固定源码回退**

`RuntimeLockV1` 必含 `schemaVersion: 1`、DSH tag/commit、platform、arch、glibc/macos floor、source、persistent URL 或 source commit、Node/pnpm/Python/Rust toolchain、files SHA-256、runtime closure、SPDX license result、CycloneDX SBOM path、profile version、contract hash、AIO semver range。官方 wheel 必须版本/平台精确匹配且可持久下载；否则 shallow fetch 固定 tag 后验证 commit 等于锁定值，在目标原生 runner 构建。

```typescript
export type RuntimeSource =
  | { kind: "official-wheel"; url: string; sha256: string }
  | { kind: "project-built-from-official-source"; tag: "dsh-v0.1.2-alpha.1"; commit: "cd5ef8148158c3a752a658978873241fdf8e2bbc" };
export type VerifiedRuntime = { platform: PlatformKey; root: string; source: RuntimeSource; files: readonly { path: string; sha256: string; executable: boolean }[] };
```

- [ ] **Step 4: 验证离线 closure 与 release-shaped runtime**

Run: `bun run runtime:resolve-current -- --out .artifacts/runtime && bun scripts/runtime/verify-runtime.ts --lock runtime-lock/dsh-v0.1.2-alpha.1.json --root .artifacts/runtime --offline && bunx vitest run tests/e2e/runtime-artifact.test.ts`

Expected: 使用官方精确 wheel或明确的 project-built 来源；断网 smoke、hash、license、SBOM、mock Provider handshake 全部 PASS。
...
### Task 6: Supervisor 初始化、DSH Home 与平台进程树（OpenSpec 4.1-4.4、4.7）
...
- [ ] **Step 1: 写真实固定版本 boot contract 测试**
...
- [ ] **Step 4: 验证真实 runtime boot/dispose**
...
- [ ] **Step 1: 写 fake-AIO/fake-DSH 控制与迟到响应测试**
...
- [ ] **Step 1: 写 gap/duplicate/remount/overload 失败测试**
...
- [ ] **Step 1: 写 allowlist、有损字段和秘密泄漏失败测试**
...
- [ ] **Step 2: 运行 adapter/credential 测试并确认失败**
...
- [ ] **Step 4: 验证 mock Provider、key rotation 与全链路脱敏**
...
- [ ] **Step 3: 接入唯一 DSH assembler 并按探测结果选择 codec**
...
- [ ] **Step 4: 验证真实 Provider 最终 payload**
...
- [ ] **Step 1: 写默认权限、full-access 非持久和 fail-closed 测试**
...
- [ ] **Step 2: 运行测试并确认 policy/status 缺失**
...
- [ ] **Step 4: 验证三种 fake capability 和当前主机 smoke**
...
### Task 14: 每平台 ZIP、原生 E2E 与支持分级（OpenSpec 2.3、7.3-7.5）
...
- Create: `E:\workspace\projects\aiohub-plugin-dsh-workspace\tests\e2e\flatpak-compat.test.ts`
- Create: `E:\workspace\projects\aiohub-plugin-dsh-workspace\tests\e2e\linux-arm64-preview.test.ts`
...
- Produces: `dsh-coding-workspace-0.1.0-win32-x64.zip`、`dsh-coding-workspace-0.1.0-linux-x64.zip`、`dsh-coding-workspace-0.1.0-darwin-arm64.zip`、`dsh-coding-workspace-0.1.0-linux-arm64-preview.zip`、SHA-256、SBOM、license bundle、support result JSON。

- [ ] **Step 1: 写最终 ZIP 验收测试**

```typescript
it("runs a release-shaped coding turn from a clean offline install", async () => {
  const app = await installFinalZip({ network: "disabled", cleanPluginData: true });
...
- [ ] **Step 2: 运行当前平台 E2E 并确认 package script 缺失**

Run: `bunx vitest run tests/e2e/installed-plugin.test.ts --testTimeout=120000`

Expected: FAIL，找不到当前平台最终 ZIP。

- [ ] **Step 3: 实现四包流水线与支持矩阵**

原生 matrix：Windows x64、Ubuntu 22.04 x64、macOS 14 arm64 为 supported；Linux arm64 只上传带 `preview` metadata 的包。每个包只含一个平台 binary map、完整 runtime closure、lock、SBOM/licenses。测试从 ZIP 全新安装开始，覆盖 offline cold start、real handshake、mock Provider coding Turn、工具子进程、取消、flush、退出、crash/interrupted、cold resume、升级、失败回退、卸载和数据保留。Linux x64 分别在 AIO `.deb` 与 `.AppImage` 宿主执行。

```yaml
matrix:
  include:
    - { os: windows-latest, platform: win32-x64, support: supported }
    - { os: ubuntu-22.04, platform: linux-x64, support: supported }
    - { os: macos-14, platform: darwin-arm64, support: supported }
    - { os: ubuntu-24.04-arm, platform: linux-arm64, support: preview }
```

- [ ] **Step 4: 增加 Flatpak 独立门禁并运行矩阵**

Flatpak 用 `flatpak run` 验证 executable、nested filesystem、shell、document portal、workspace access、bwrap/Landlock；失败输出 `unsupported` 或 `preview` 和具体 capability，不增加宽泛 filesystem 权限。Linux ARM64 只运行 artifact/hash/handshake/native smoke，断言发布 metadata 不含 `supported`。

Run: `bun run package:platform && bunx vitest run tests/e2e/installed-plugin.test.ts`

Expected: 三个 supported lane 全门禁 PASS；Linux ARM64 preview smoke PASS；Flatpak 只有独立门禁全过才输出 supported。

- [ ] **Step 5: 提交**

```bash
git add scripts/package-platform.ts .github/workflows tests/e2e tests/fixtures/mock-provider.ts
git commit -m "build: package and gate DSH runtime platforms"
```

### Task 15: 发布文档、兼容边界与全量门禁（OpenSpec 7.6）
...
- [ ] **Step 1: 写文档/发布一致性失败测试**

```typescript
it("documents exactly the machine-readable platform and trust matrix", async () => {
  const report = await verifyRelease(".");
  expect(report.supported).toEqual(["win32-x64", "linux-x64", "darwin-arm64"]);
  expect(report.preview).toEqual(["linux-arm64"]);
  expect(report.failures).toEqual([]);
});
```

- [ ] **Step 2: 运行 release verifier 并确认文档缺失**
...
文档必须说明：安装与开发 junction、四平台分级、glibc/macOS floor、Flatpak 判定、runtime 来源/SBOM/license/hash、DSH alpha 与未审计 sandbox 边界、同用户凭据读取模型、诊断脱敏、崩溃 interrupted/不重放、数据保留/升级/回退/卸载、VCP/OpenAI-compatible 基线、unsupported Provider 错误、`{{Nova}}` 保真、后续 `CredentialProvider`/`ModelTransport`/global Capability Runtime 替换接口。明确排除 UI、market 与 RAG/Knowledge。
...
- [ ] **Step 4: 执行全量静态、单元、合同和当前平台 release gate**
...
- Tasks 1-3 建立其他插件任务依赖的仓库、Facade 与协议；Task 5 产出 runtime；Tasks 6-7 产出 Supervisor；Tasks 8-13 完成 bridge 和策略；Task 14 才建立最终平台包；Task 15 收口发布门禁。
- 每个任务完成后只提交该任务列出的文件。独立插件仓库与 AIO 主仓库分别提交，不跨仓形成单个 commit。
