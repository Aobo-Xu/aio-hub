...
- [x] 10.3 通过生产 AIO 插件安装与 resident Sidecar IPC 运行集成测试；使用隔离 app-data 核验真实 workspace/session、snapshot 恢复、interaction、cancel、进程树清理、升级/回退 hook、卸载与数据保留（原枚举场景双 lane 8/8+6/6，ZIP `1f9db8f9`，证据 `.dev-data/dsh-hostcap-r1b|r2b/artifacts/`；后补 wire 操作的新增 E2E 证据独立列为 10.3c，避免把旧结果冒充新接线验证）
- [x] 10.3a 将 Tasks 5–12 已交付的 workspace/session/search/history/queue/restart/terminal/preset/dynamic/attachment/summary 能力补入协议单一事实源与 RuntimeFacade DTO；所有 mutation 保持 request identity、generation、lease 与 capability fail-closed 语义（生成协议 hash `96af8af6…`；RuntimeState 12 态；RuntimeFacade `query()` 统一消费语义命令）
- [x] 10.3b 将 10.3a 的命令通过 Supervisor resident 路由接到生产 Host bridge，并为 rc.1 未提供的 preset/dynamic/browser-half 等能力返回结构化 unavailable，不以版本硬编码或私有 DSH API 模拟能力（typed Host read/mutation allowlist + generation/capability/lease/requestId fencing；release Host probe 已通过）
- [x] 10.3c 通过协议生成测试、Host dispatch 测试和生产 IPC E2E 逐行证明 R1、R4–R7、R12、R15–R17、R19–R20 可达；不得以 bridge 单测替代生产链路证据（协议、dispatch、Supervisor stdio 与 release-shaped Host probe 均通过；此前 WebView2 `HRESULT 0x80070057` 确认为临时环境故障未再复现——修复 supervisor capability 映射缺陷（camelCase wire kind 泄漏进 capability gate）并重建 ZIP `d23a70a3`（verify-release 0 failures、probe smoke ready 27 capabilities）后，扩展后 AIO `dsh-host-capability` spec 生产 IPC E2E **7/7 通过**（含 mutation fence stale-lease 拒绝、权威快照真实 DSH facts、cold recovery、interaction fail-closed），`dsh-runtime-native` 回归 **8/8 通过**；证据 `.dev-data/dsh-hostcap-r6b/`、`.dev-data/dsh-native-r6c/`）
- [x] 10.4 重跑 Coding Workstation Host Gate；仅当 R1–R17、R19–R20 全部通过、R18 明确 fail closed、R8 包含真实 DSH facts 时更新门禁报告为通过（协议/Facade/Supervisor/Host bridge 生产实现面 21/21 pass；R18 明确 fail-closed，R8 为真实 DSH facts；后续 10.5 已取得非豁免 native E2E 通过）
- [x] 10.5 在已记录的离线策略下仅运行一次 Windows native E2E 里程碑；只有 build/package/artifact/smoke 成功后的受控 pre-test 基础设施故障可临时豁免，并必须保持 `gatePassed=false`、`formalReleaseBlocked=true`，不得写成产品 E2E 通过（GitHub Actions 离线门禁 run `34280645166`（分支 `codex/add-dsh-host-capability-foundation`，AIO head `17eb15cb1`，插件 pinned `5f9af2dd`，ZIP `d23a70a3…` build/package/verifier/probe 全通过后）完整执行 dsh-runtime-native preset：**8/8 通过（31.1s）**，分类器 `status=passed、gatePassed=true、temporaryWaiver=false、reasonCode=E2E_PASSED`，无需豁免；本机隔离双 lane 此前已同样通过（7/7+8/8）。执行模型偏差：接口级全阻断在 Windows block 优先于 allow 的语义下无法保留 runner 生命线（run #10 于 lane 健康执行中被平台取消），收敛为被测面程序级封禁（aiohub/msedgewebview2/supervisor/runtime/rg/node/bun/msedgedriver 无外联，以被封禁 node 实测验证），详见 gate 报告增补）
- [x] 10.6 同步 OpenSpec tasks、Superpowers 实施/验证报告及跨仓 commit/provenance 引用，并核验在已验收 Host Gate 未完全满足前 Coding Workstation 仍保持阻塞（已同步为 Host 契约/实现 21/21、GitHub native E2E `E2E_PASSED`、`temporaryWaiver=false`；Coding Workstation UI 前置阻塞已解除）
