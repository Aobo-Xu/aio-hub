...
- [ ] 10.5 在已记录的离线策略下仅运行一次 Windows native E2E 里程碑；只有 build/package/artifact/smoke 成功后的受控 pre-test 基础设施故障可临时豁免，并必须保持 `gatePassed=false`、`formalReleaseBlocked=true`，不得写成产品 E2E 通过（最终 ZIP `d23a70a3…` 的 build/package/verifier/release Host probe 全通过；本机隔离双 lane 已在记录的离线策略下真实通过（dsh-host-capability 7/7 + dsh-runtime-native 8/8），WebView2 `0x80070057` 确认为临时环境故障；但按既定策略本机结果不替代 GitHub 离线 CI 门禁，GitHub 最新仍为 2026-09-04 旧提交失败运行，推送 `aobo-validation` 重跑 CI 待用户授权，`formalReleaseBlocked=true`，保持未勾选）
