...
- [x] 7.4 Add a required Windows GitHub Actions lane that builds the AIO debug binary, supplies the Tauri frontend origin, isolated app-data root and WebDriver port, then runs the production-IPC native E2E with network disabled during the runtime test stage and uploads only redacted reports, checksums, and test results. GitHub run #10 completed all hard artifact gates and recorded a temporary pre-session WebDriver infrastructure waiver; `gatePassed=false` and formal release remains blocked until a real native E2E pass.
